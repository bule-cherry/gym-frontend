        ...
        <el-row>
          ...
          <el-col :span="12" :offset="0">
            <el-form-item prop="teacherName" label="教练">
              <el-select style="width: 100%" v-model="addModel.teacherName" class="m-2" placeholder="请选择教练"
                size="default" @change="selectTeach">
    ...
    nextTick(() => {
      ...
      if (row?.image) {
        ...
        fileList.value.push({
          name: '封面图',
          url: row.image,
          status: 'success', // ✅ 告诉 Element Plus 这个文件是已上传状态
          uid: Date.now()    // ✅ 唯一标识符（随便一个数字都可以）
      })
      }
    });
  }
