<template>
    <el-main>
        ...
        <el-form :model="listParm" :inline="true" size="default">
            <el-form-item>
                <el-input v-model="listParm.name" label="请输入商品名称">
                </el-input>
            </el-form-item>
            <el-form-item>
        ...
        <el-table :height="orderTableHeight" :data="tableData.list" border stripe>
            <el-table-column prop="image" width="90" label="商品图片" align="center">
                <template #default="scope">
                    <el-image style="width: 60px; height: 60px; border-radius: 
50%" :src="scope.row.image"></el-image>
                </template>
            </el-table-column>
        ...
        <el-pagination @size-change="orderSizeChange" @current-change="orderCurrentChange"
            :current-page.sync="listParm.currentPage" :page-sizes="[20, 40, 80, 100]" :page-size="listParm.pageSize"
        ...
        <el-drawer :modal="false" custom-class="faDrawer" :append-to-body="true" :modal-append-to-body="false"
            ...
            <template #title>
                ...
                <el-button size="small" @click="innerDrawerShow">选择商品
                </el-button>
                <el-drawer custom-class="childDrawer" size="50%" v-model="innerDrawer" title="商品列表"
                    ...
                    <el-form style="margin-top: 10px" :model="goodsParm" :inline="true" size="default">
                        <el-form-item>
                            <el-input v-model="goodsParm.name" placeholder="请输入商品名称"></el-input>
                        </el-form-item>
                        <el-form-item>
                    ...
                    <el-pagination @size-change="goodsSizeChange" @current-change="goodsCurrentChange"
                        :current-page.sync="goodsParm.currentPage" :page-sizes="[20, 40, 80, 100]"
                        :page-size="goodsParm.pageSize" layout="total, sizes, prev, pager, next, jumper"
                        :total="goodsParm.total" background>
                    </el-pagination>
                </el-drawer>
            </template>
            <template #default>
...
const { faDrawer, cancelClick, confirmClick, order,
    deleteCarBtn } =
